...
(function () {
  ...
  window.addEventListener('unhandledrejection', function (event) {
    debugLog('unhandledrejection fired: ' + event.reason); // Keep track of rejected promises by adding them to the list.
    ...
    if (!logRejectionTimeoutId) {
      logRejectionTimeoutId = setTimeout(logRejectedPromises, PROMISE_REJECTION_LOGGING_DELAY);
    }
  });
  window.addEventListener('rejectionhandled', function (event) {
    debugLog('rejectionhandled fired: ' + event.reason); // If a previously rejected promise is handled, remove it from the list.
  ...
  function lazyLoadWCPolyfillsIfNecessary(callback) {
    ...
    if (!(HOVERBOARD.Util.getChromeVersion() && HOVERBOARD.Util.getChromeVersion() >= 46 || HOVERBOARD.Util.getFirefoxVersion() && HOVERBOARD.Util.getFirefoxVersion() >= 40)) {
  ...
  window.addEventListener('offline', function () {
    if (HOVERBOARD.Elements && HOVERBOARD.Elements.Template.$.toast) {
      HOVERBOARD.Elements.Template.$.toast.showMessage('You can still work offline.');
    }
  ...
  window.addEventListener('beforeinstallprompt', function (event) {
    ...
    event.userChoice.then(function (choiceResult) {
      ...
      HOVERBOARD.Analytics.trackEvent('installprompt', choiceResult.outcome, choiceResult.platform);
    });
  });
