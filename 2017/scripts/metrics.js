...
HOVERBOARD.Analytics = HOVERBOARD.Analytics || function (exports) {
  ...
  function Analytics() {
    ...
    this.initTrackerReadyState(); // Tracks the initial pageview.

    this.trackPageView(); // Tracks perf events that were marked in the template code. Changes here
  ...
  Analytics.prototype.NULL_DIMENSION = '(not set)'; // A map from each custom dimension name to its index in Google Analytics.
  ...
  Analytics.prototype.requiredDimensions = [Analytics.prototype.customDimensions.SIGNED_IN, Analytics.prototype.customDimensions.ONLINE, Analytics.prototype.customDimensions.SERVICE_WORKER_STATUS, Analytics.prototype.customDimensions.NOTIFICATION_PERMISSION];
  /**
  ...
  Analytics.prototype.initTrackerReadyState = function () {
    this.readyState_ = HOVERBOARD.Deferred.createDeferred(); // In the event of an error or a failure in the auth code, we set a
    ...
    setTimeout(function () {
      this.readyState_.resolve(); // Tracks that this happened and when it happened.

      this.trackEvent('analytics', 'timeout', this.READY_STATE_TIMEOUT_, window.performance && Math.round(window.performance.now()));
  ...
  Analytics.prototype.trackPerf = function (category, variable, time, opt_label, opt_maxTime, opt_obj) {
    this.waitForTrackerReady().then(function () {
      ...
      opt_label = opt_label || this.NULL_DIMENSION; // Sets the time value as a dimension so it can be more usefully reported
      // on (e.g. median, distribution, etc).

      opt_obj = opt_obj || {};
      opt_obj[this.customDimensions.METRIC_VALUE] = time; // Sends an event and a timing hit. We keep the timing hit for historical
  ...
  Analytics.prototype.trackError = function (location, message) {
    this.waitForTrackerReady().then(function () {
      ga('send', 'event', 'error', location, String(message)); // Note: GA has exception type but it does not show up in realtime so catching
  ...
  Analytics.prototype.trackServiceWorkerControlled = function () {
    ...
    this.updateTracker(this.customDimensions.SERVICE_WORKER_STATUS, serviceworkerStatus);
    ...
    if (serviceworkerStatus === 'unsupported') {
      ...
      this.trackEvent('serviceworker', 'controlled', serviceworkerStatus === 'controlled');
    }
  };
  /**
  ...
  Analytics.prototype.getNotificationPermission = function () {
    return exports.Notification ? exports.Notification.permission : 'unsupported';
  };
